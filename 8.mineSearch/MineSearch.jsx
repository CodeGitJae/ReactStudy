   ...
   switch (action.type) {
      ...
      case OPEN_CELL: {
         ...
         const checkAround = (row, cell) => {
            if (row < 0 || row >= tableData.length || cell < 0 || cell >= tableData[0].length) {  // 상하좌우 없는칸은 안 열기
               return;
            }
            if ([CODE.OPENED, CODE.FLAG, CODE.FLAG_MINE, CODE.QUESTION_MINE, CODE.QUESTION].includes(tableData[row][cell])) { // 닫힌 칸만 열기
               return;
            }
            ...
            if (tableData[row][cell] === CODE.NORMAL) {   // 내 칸이 닫힌 칸인 경우엔 카운트 증가
               openedCount += 1;
            }
...
const  MineSearch = () => {
   ...
   const value = useMemo(() => ({ tableData, halted, dispatch }), [tableData, halted]);  // Context API 사용할 때 useMemo를 통해 cache를 해야 성능 저하가 일어나지 않는다.
